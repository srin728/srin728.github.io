...
def authors(item):
    ...
    s = "，".join(esc(x) for x in xs) if xs else esc(item.get("speaker", ""))
    if item.get("affiliation"):
        s += f'（{esc(item["affiliation"])}）'
    return s
...
def talks_html(items):
    ...
    for item in ordered:
        ...
        a = authors(item)
        if a:
            parts.append(f'<span class="entry-authors">{a}</span>')
        t = link(str(item.get("url", "")), str(item.get("title", "Untitled talk")))
        parts.append(f'<span class="entry-talk-title">「{t}」</span>')
        venue = "，".join(
            esc(x) for x in (
                item.get("event", ""),
                item.get("location", ""),
        ...
        if venue:
            parts.append(f'<span class="entry-venue">{venue}</span>')
        note = esc(item.get("note", ""))
        note_html = f'<p class="entry-note">{note}</p>' if note else ""
        rows.append('<li class="entry"><p class="entry-citation">' + "．".join(parts) + "．</p>" + note_html + "</li>")
    return '<ol class="entry-list numbered-list" reversed>\n' + "\n".join(rows) + "\n</ol>"
...
def updates_html(items):
    ...
    for item in ordered:
        ...
        rows.append(f'<li class="entry"><p class="entry-title">{text}</p><p class="entry-meta">{date}</p></li>')
...
def expected_outputs(root):
    ...
    landing = (
        ...
        '<h2>補足情報</h2>'
        '<p>上のボタンから表示する内容を選択してください。</p>'
        '</section>'
    )
    note = str(data.get("coauthor_talks_note", ""))
    note_html = f'<p class="section-note">{esc(note)}</p>' if note else ""
    talk_body = (
        '<section id="coauthor-talks"><h2>共著者による発表</h2>'
        + note_html + talks_html(data.get("coauthor_talks") or []) + '</section>'
    )
    update_body = (
        '<section id="updates"><h2>近況報告</h2>'
        + updates_html(data.get("updates") or []) + '</section>'
    )

    return {
        root / "supplement.html": page(title, title, intro, base + "supplement.html", landing, updated),
        root / "supplement-talks.html": page(
            "共著者による発表 | " + title,
            "共著者による発表",
            "共著者による研究発表を掲載しています。",
            base + "supplement-talks.html",
            talk_body,
            updated
        ),
        root / "supplement-updates.html": page(
            "近況報告 | " + title,
            "近況報告",
            "研究活動に関する近況報告を掲載しています。",
            base + "supplement-updates.html",
            update_body,
            updated
        ),
    }
